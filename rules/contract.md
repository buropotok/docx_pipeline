...
Базовые принципы (как контракт реконструктора)
1) Управляемость

Только SDT(tag=id) = “управляемый блок”.

Любой блок без такого маркера — никогда не удаляем и не перестраиваем, только passthrough.

Следствие: deleted:true применим только к управляемым id. Если id не найден в XML → это ошибка/варнинг (в зависимости от режима), но не повод что-то удалять.

2) Patch-mode по умолчанию

JSON может быть неполным. Это нормально.

Отсутствие элемента в JSON ничего не значит.

3) Минимальные изменения

Изменяем только то, что указано в overrides.

Никаких “Normal по умолчанию”, никаких “если не хватает — додумали”. Это ключ к “no synthetic”.

Порядок применения патчей (очень важно)

Чтобы избежать конфликтов и недетерминизма, фиксируем детерминированный порядок:

Построить индекс: id -> sdtNode по текущему donor document.xml.

Удаления: для всех patch-объектов с deleted:true

удалить соответствующий sdt wrapper целиком (а не внутренний w:p).

Вставки (operations):

найти anchor_id (тоже SDT),

вычислить контейнер (body или sdtContent ячейки),

вставить новый SDT рядом (before/after).

базовый контент нового элемента:

либо “clone from anchor” (строго копирование donor XML якоря),

либо “from template” (если тип другой).

Модификации: применить overrides к оставшимся / новым управляемым элементам.

Почему так:

если сначала модифицировать, а потом удалить — лишняя работа и риск побочных эффектов;

если вставлять до удаления — якоря могут исчезнуть, и вставка станет двусмысленной.

Вставки: модель данных, которая не ломается

Ваши поля:

anchor_id

insert_after (или расширить до position: before|after)

Я бы добавил ровно одно поле для детерминизма и дебага:

op_id (или new_id уже достаточно) — уникальный id операции (можно равным new_id)

И правило:

new_id должен быть уникален в документе, и реконструктор должен валидировать отсутствие коллизий (иначе будете “тихо” патчить не то).

Про “new_id: p_2.1”

Работает отлично, если зафиксировать:

p_2.1 — это не “математика”, это просто строковый id.

порядок нескольких вставок после одного anchor определяется сортировкой new_id (или явным order).

Иначе появится недетерминизм: если две вставки “после p_2”, их порядок станет зависеть от порядка обхода массива.

Таблицы и новая иерархия

Ключевой момент: маркер должен быть block-level и одинаково работать для w:p и w:tbl. SDT как раз даёт это.

Когда добавите таблицы:

вы сможете маркировать таблицу целиком,

и/или маркировать параграфы внутри ячеек (если нужно гранулярно).

Но важно заранее решить: “управляемость” на каком уровне?

только block-level в body (проще),

или ещё и внутри ячеек (нужно для перевода текста в таблицах).

С вашим patch-mode это решается просто: вы маркируете то, что хотите уметь патчить.

Подводные камни, которые стоит зафиксировать сейчас

Никогда не “пересобирать” w:p целиком, если override не требует менять текст/раны.
Иначе вы начнёте терять ссылки/поля/закладки внутри абзаца. Для Word это частая боль.

Не трогать sectPr: секции должны оставаться донорскими, иначе ломается пагинация/колонтитулы.

Режим ошибок:

anchor_id не найден → ошибка уровня патча (операция не применена).

deleted:true, но id не найден → тоже ошибка/варнинг.

коллизия new_id → ошибка (иначе патч непредсказуем).

Версионирование схемы: как только вводите SDT tag-id как “source of truth”, парсер должен предпочитать tag вместо счетчика. Это делает id стабильными между прогонами.